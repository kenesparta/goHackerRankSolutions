...
func countApplesAndOranges(s int32, t int32, a int32, b int32, apples []int32, oranges []int32) {
	var routeApples []int32
	var routeOranges []int32
	for _, i := range apples {
		routeApples = append(routeApples, a+i)
	}
	for _, i := range oranges {
		routeOranges = append(routeOranges, b+i)
	}

	// Calculate if the orange or apple is in the interval [s , t]
	var numApples int32
	var numOranges int32
	for _, i := range routeApples {
		if i >= s && i <= t {
			numApples++
		}
	}
	for _, i := range routeOranges {
		if i >= s && i <= t {
			numOranges++
		}
	}
